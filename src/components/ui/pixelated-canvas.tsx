...
const defaultProps: Partial<PixelatedCanvasProps> = {
  width: 400,
  height: 500,
  cellSize: 3,
  dotScale: 0.9,
...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = (props) => {
  const {
    src,
    width,
    height,
    ...
    fadeOnLeave,
    fadeSpeed,
  } = { ...defaultProps, ...props } as Required<PixelatedCanvasProps>;
  ...
  React.useEffect(() => {
    ...
    // Will hold cleanup actions to run when image is removed.
    ...
    const compute = () => {
      ...
      const displayWidth = width ?? img.naturalWidth ?? 1;
      const displayHeight = height ?? img.naturalHeight ?? 1;
      ...
      try {
        ...
        // Cross-origin or other issue — fallback to just drawing the image to the visible canvas.
    ...
    img.onload = () => {
      ...
      // If interactivity is disabled, render a static pixelated snapshot once.
      if (!interactive) {
      ...
      // Interactive mode: pointer handlers + animation loop
  ...
  return <canvas ref={canvasRef} className={className} aria-label="Pixelated rendering of source image" role="img" style={{ display: "block" }} />;
};
